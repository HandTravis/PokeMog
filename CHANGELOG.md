...
- `Base.metadata.create_all` on startup as safety net
...
- `test_seed.py` — pure function tests for `get_generation` and `get_evolution_stage`; HTTP behaviour tests for `seed_pokemon`
...
- `backend.yaml` — Deployment + ClusterIP Service with `/health` liveness and readiness probes, `imagePullPolicy: Never` for local images
...
- `imagePullPolicy: Never` set on backend and frontend to use locally built images
